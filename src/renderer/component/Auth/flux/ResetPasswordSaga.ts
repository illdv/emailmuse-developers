...
function* onResetPasswordsword(action: {payload: {email: string, token: string, password: string, passwordConfirmation: string}}): IterableIterator<any> {
  try {
...
export function* resetPasswordSaga(): IterableIterator<any> {
  while (true) {
    ...
    yield call(onResetPasswordsword, action);
  }
}
