...
const getPathWithNameJsonFile = (filename: string): string =>
  path.join(pathToLogs, filename + '.json');
...
namespace Logger {
  ...
  const setLogsForFile = (logs: IJsonLogFormat[]): string =>
    JSON.stringify({ logs });

  const selectLogsFromFile = (file: string): IJsonLogFormat[] =>
    JSON.parse(file).logs || [];

  const filterLogsByUnixTime = (
    logs: IJsonLogFormat[],
    unix: number,
  ): IJsonLogFormat[] =>
  ...
  const setLogs = (logs: IJsonLogFormat[]) =>
    (queueThen = queueThen.then((arr: IJsonLogFormat[]) => [...arr, ...logs]));

  const saveToFile = () =>
    (queueThen = queueThen.then(
      (logs: IJsonLogFormat[]) =>
        new Promise(resolve =>
  ...
  export function writeLog(log: ILog) {
    ...
    setLogs([jsonLogFormat]).then((arr: IJsonLogFormat[]) => {
      saveToFile();
    });
  }
